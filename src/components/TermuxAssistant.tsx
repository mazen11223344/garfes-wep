  ...
  const processUserInput = async (text: string, imageFile?: File | null) => {
    ...
    try {
      ...
      if (reminderDetails) {
        ...
        addMessage("assistant", "تم تحليل الصورة.", undefined, new Date(), chartResponse);
        ...
        const { responseForUser, updatedMemory } = await analyzeAndStoreMemory(arabicText, assistantMemory);
        setAssistantMemory(updatedMemory);
        addMessage("assistant", responseForUser);
      }
    } catch (error) {
